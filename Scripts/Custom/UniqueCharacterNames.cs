...
namespace Felladrin.Automations
{
    public static class UniqueCharacterNames
    {
        ...
        static void OnLogin(LoginEventArgs args)
        {
            ...
            from.SendMessage(33, "Your character name '{0}' is already in use by another player or it has been marked as invalid. Please choose a new one.", from.Name);
        ...
        static bool HasValidName(Mobile m)
        {
            ...
            foreach (Mobile otherPlayer in World.Mobiles.Values)
                if (otherPlayer is PlayerMobile && otherPlayer != m && otherPlayer.RawName != null && m.RawName != null && otherPlayer.RawName.ToLower() == m.RawName.ToLower())
                    return false;
