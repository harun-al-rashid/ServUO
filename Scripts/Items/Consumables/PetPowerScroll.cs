    ...
    public class PetPowerScroll : SpecialScroll
    {
        ...
        * You can view your maximum statistic value in your statistics window. */
        ...
        public override string DefaultTitle
        {
            get
            {
                return String.Format("<basefont color=#FFFFFF>Pet Scroll ({0} Skill):</basefont>", this.Value);
            }
        }
        public static PetPowerScroll CreateRandom(int min, int max)
        {
            ...
            return new PetPowerScroll(Skills[Utility.Random(Skills.Count)], 100 + (Utility.RandomMinMax(min, max) * 5));
        }

        /*public static PetPowerScroll CreateRandomNoCraft(int min, int max)
        {
            min /= 5;
            max /= 5;
            ...
            return new PetPowerScroll(skillName, 100 + (Utility.RandomMinMax(min, max) * 5));
        }*/
        ...
        {
            from.SendLocalizedMessage(1063559); // Target the Pet you wish to exalt.
            from.Target = new PetScrollTarget(this);
        }

        private class PetScrollTarget : Target
        {
            ...
            public PetScrollTarget(PetPowerScroll scroll)
            ...
            protected override void OnTarget(Mobile from, object targ)
            {
                ...
                if (targ is BaseCreature)
                {
                    ...
                    if (bc.ControlMaster != from) { from.SendMessage("Creature must be your pet"); return; }
                    //List<Mobile> owners = bc.Owners;
                    //bool ispet = false;
                    //foreach(var mob in owners) { if (mob == from) ispet = true; }
                    //if (!ispet) { from.SendMessage("Creature must be your pet"); }
                    if (!bc.Alive) { from.SendMessage("Your target must be alive"); return; }

                    SkillName skill = m_scroll.Skill;

                    if(skill == SkillName.Magery)
                    {
                        bc.Skills.Magery.Cap = (int)m_scroll.Value;
                    }else if(skill == SkillName.EvalInt)
                        {
                        bc.Skills.EvalInt.Cap = (int)m_scroll.Value;
                    }
                    else if (skill == SkillName.Meditation)
                    {
                        bc.Skills.Meditation.Cap = (int)m_scroll.Value;
                    }
                    else if (skill == SkillName.Wrestling)
                    {
                        bc.Skills.Wrestling.Cap = (int)m_scroll.Value;
                    }
                    else if (skill == SkillName.Tactics)
                    {
                        bc.Skills.Tactics.Cap = (int)m_scroll.Value;
                    }
                    else if (skill == SkillName.MagicResist)
                    {
                        bc.Skills.MagicResist.Cap = (int)m_scroll.Value;
                    }

                    from.SendMessage("Your Pet experiences a surge of Power!"); // You feel a surge of magic as the scroll enhances your ~1_type~!

                    

                    Effects.SendLocationParticles(EffectItem.Create(bc.Location, bc.Map, EffectItem.DefaultDuration), 0, 0, 0, 0, 0, 5060, 0);
                    Effects.PlaySound(bc.Location, bc.Map, 0x243);

                    Effects.SendMovingParticles(new Entity(Serial.Zero, new Point3D(bc.X - 6, bc.Y - 6, bc.Z + 15), bc.Map), bc, 0x36D4, 7, 0, false, true, 0x497, 0, 9502, 1, 0, (EffectLayer)255, 0x100);
                    Effects.SendMovingParticles(new Entity(Serial.Zero, new Point3D(bc.X - 4, bc.Y - 6, bc.Z + 15), bc.Map), bc, 0x36D4, 7, 0, false, true, 0x497, 0, 9502, 1, 0, (EffectLayer)255, 0x100);
                    Effects.SendMovingParticles(new Entity(Serial.Zero, new Point3D(bc.X - 6, bc.Y - 4, bc.Z + 15), bc.Map), bc, 0x36D4, 7, 0, false, true, 0x497, 0, 9502, 1, 0, (EffectLayer)255, 0x100);

                    Effects.SendTargetParticles(bc, 0x375A, 35, 90, 0x00, 0x00, 9502, (EffectLayer)255, 0x100);
